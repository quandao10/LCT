...
class LatentDataset(torch.utils.data.Dataset):
    def __init__(self, path: Path, transform=None):
        self.path = path
        self.np_paths = natsorted(path.rglob("*.npy"))
    ...
    def __getitem__(self, idx):
        x = np.load(self.np_paths[idx], allow_pickle=True)
        latent = torch.from_numpy(x)
        if self.transform is not None:
            latent = self.transform
        return latent, torch.tensor(0)
...
def main(args):
    ...
    if args.use_wavelet:
        model = DiM_models[args.model](in_channels=16, 
                                        img_resolution=16, 
                                        learn_sigma = args.learn_sigma, 
                                        pe_type=args.pe_type, 
                                        block_type=args.block_type, 
                                        using_dct=args.using_dct, 
                                        cpe_dilated=args.use_dilated,
                                        use_wavelet=args.use_wavelet).to(device)
    else:
        model = DiM_models[args.model](learn_sigma = args.learn_sigma, 
                                        pe_type=args.pe_type, 
                                        block_type=args.block_type, 
                                        using_dct=args.using_dct, 
                                        cpe_dilated=args.use_dilated).to(device)
    ...
    # Setup resume
    if args.resume:
        checkpoint = find_model(args.resume, False)
    ...
    # dataset = LatentDataset(args.datadir)
    ...
    for epoch in range(init_epoch, args.epochs+1):
        ...
        #     and epoch >= args.eval_start_epoch
        #     or epoch == args.epochs - 1
        # ):
        #     with torch.no_grad():
        #         diffusion_eval = create_diffusion(str(250), learn_sigma=args.learn_sigma)
        #         # vae = AutoencoderKL.from_pretrained(f"./vim/stabilityai/sd-vae-ft-mse").to(device)
        #         ref_dir = Path(args.eval_refdir)
        #         if ref_dir.exists():
        #             n = args.eval_bs
        #             using_cfg = args.eval_cfg_scale > 1.0
        #             global_eval_batch_size = n * dist.get_world_size()
        #             total_samples = int(
        #                 math.ceil(args.eval_nsamples / global_eval_batch_size)
        #                 * global_eval_batch_size
        #             )
        #             samples_needed_this_gpu = int(
        #                 total_samples // dist.get_world_size()
        #             )
        ...
        #             for _ in pbar:
        #                 # Sample inputs:
        #                 z = torch.randn(n, x.shape[1], x.shape[2], x.shape[3], device=device)
        ...
        #                     result_dict, run_dir=p.as_posix(), snapshot_pkl=p.as_posix()
        #                 )
        #                 del z, result_dict, samples
        #             dist.barrier()
        #             gc.collect()
        #             torch.cuda.empty_cache()
        #         else:
        #             logger.info(f"Reference directory {ref_dir} does not exist, skip eval")
