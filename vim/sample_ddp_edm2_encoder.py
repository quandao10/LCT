...
def main(args):
    ...
    if args.use_wavelet:
        model = DiM_models[args.model](in_channels=16, img_resolution=16, learn_sigma = args.learn_sigma, pe_type=args.pe_type, block_type=args.block_type, using_dct=args.using_dct, cpe_dilated=args.use_dilated).to(device)
    else:
        model = DiM_models[args.model](img_resolution=latent_size, learn_sigma = args.learn_sigma, pe_type=args.pe_type, block_type=args.block_type, cpe_dilated=args.use_dilated).to(device)
    ...
    if args.use_wavelet:
        print(f"Using Wavelet Decomposition for training")
    ...
    for _ in pbar:
        # Sample inputs:
        if args.use_wavelet:
            z = torch.randn(n, model.in_channels, latent_size//2, latent_size//2, device=device)
        else:
        ...
        if args.use_wavelet:
            samples = iwt((samples[:, :4], [torch.stack((samples[:, 4:8], samples[:, 8:12], samples[:, 12:16]), dim=2)]))
        ...
        for i, sample in enumerate(samples):
            ...
            Image.fromarray(sample.permute(1, 2, 0).cpu().numpy()).save(f"{sample_folder_dir}/{index:06d}.jpg")
