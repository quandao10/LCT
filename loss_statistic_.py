...
for num_scales, ckpt_file in ckpt_dict.items():
    ...
    os.makedirs(os.path.join('./loss_diffs', f'{num_scales}nfe_ckpt_{ckpt_file}'), exist_ok=True)
    for scale_value in tqdm(range(num_scales - 2, -1, -1), desc=f'{num_scales} nfe, ckpt {ckpt_file}'):
        os.makedirs(os.path.join('./loss_diffs', f'{num_scales}nfe_ckpt_{ckpt_file}', f'{scale_value}'), exist_ok=True)
        for i, (x, y) in enumerate(loader):
            ...
            with open(os.path.join('./loss_diffs', f'{num_scales}nfe_ckpt_{ckpt_file}', f'{scale_value}', f'{i}.npy'), 'wb') as f:
