...
do
    ...
    do
        CUDA_VISIBLE_DEVICES=0 torchrun --nnodes=1 --rdzv_endpoint 0.0.0.0:$MASTER_PORT --nproc_per_node=1 test_cm_latent_ddp.py \
                --ckpt ./results/latent_celeb256/large_dhariwal_unet_cauchy_no_grad_norm_diff_0.40_newdiff_fix_5_bs128_othard_newc_allnonscalinglayernorm/checkpoints/000${epoch}.pt \
                --seed 42 \
            ...
            python eval_toolbox/calc_metrics.py \
                ...
                --mirror 1 \
                --gen_data generated_samples/latent_celeb256/explarge_dhariwal_unet_cauchy_no_grad_norm_diff_0.40_newdiff_fix_5_bs128_othard_newc_allnonscalinglayernorm_ep000${epoch}_${ema} \
                --img_resolution 256
    done
done
