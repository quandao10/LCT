...
for epoch in 1400
do
    # for ema in "ema_0.999" "ema" "ema_0.99993" "ema_0.99994" "ema_0.99995" "ema_0.99997" "ema_0.9999432189950708"
    ...
    do
        CUDA_VISIBLE_DEVICES=0 torchrun --nnodes=1 --rdzv_endpoint 0.0.0.0:$MASTER_PORT --nproc_per_node=1 test_cm_latent_ddp.py \
                --ckpt ./results/latent_celeb256/large_dhariwal_unet_cauchy_no_grad_norm_diff_0.75_newdiff_fix_5_bs128_othard_newc_allnonscalinginstancenorm/checkpoints/000${epoch}.pt \
                --seed 42 \
                ...
                --compute-fid \
                --ema $ema \
                --last-norm-type non-scaling-instance-norm \
                --block-norm-type non-scaling-instance-norm \

            python eval_toolbox/calc_metrics.py \
                --metrics pr50k3_full \
                --data ~/datasets/celeba_256_jpg \
                --mirror 1 \
                --gen_data generated_samples/latent_celeb256/explarge_dhariwal_unet_cauchy_no_grad_norm_diff_0.75_newdiff_fix_5_bs128_othard_newc_allnonscalinginstancenorm_ep000${epoch}_${ema} \
                --img_resolution 256
    done
done
