...
do
        CUDA_VISIBLE_DEVICES=0 torchrun --nnodes=1 --rdzv_endpoint 0.0.0.0:$MASTER_PORT --nproc_per_node=1 test_cm_latent_ddp.py \
                --ckpt results/latent_celeb256/dhariwal_unet_ict_large_batchsize_lr_decay_1k6_epoch_normalize_0.5_1280_nogradnorm_gemanmcclure_positional/checkpoints/000${epoch}.pt \
                --seed 42 \
...
do
    python eval_toolbox/calc_metrics.py \
        ...
        --mirror 1 \
        --gen_data generated_samples/latent_celeb256/expdhariwal_unet_ict_large_batchsize_lr_decay_1k6_epoch_normalize_0.5_1280_nogradnorm_gemanmcclure_positional_ep000${epoch} \
        --img_resolution 256
done
